...
namespace OpcWServiceServer.Implement
{
    ...
    class AsyncSocketServer : ISocketServer
    {
        ...
        public void StartListening()
        {
            ...
            IPEndPoint localEndPoint = new IPEndPoint(ipAddress, Port);//11001
        ...
        {
            try {
                if (_client == null || _client.Connected == false) return;
                Socket handler = _client;
                var jsonText = JsonConvert.SerializeObject(objData) + charEof;
        ...
        private void DoTaskFor_DataReceive(string st)
        {
            try
            {
                string txtJson = st.Replace(charEof, "");
                ...
                switch (requestType)
                {
                    ...
                    case (int)RequestType.RemoveServer:
                        ...
                        break;
                    default:
                        OnReceiveData(RequestType.Error, new ErrorResponseModel { RequestType = RequestType.Error, Message = "Unnownk RequestType: " + requestType });
                        break;
                }
            }
            catch (Exception e)
